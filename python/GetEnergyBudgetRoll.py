...
if len(sys.argv)==5:
    ...
    sys.exit('GetEnergyBudget.py prefix U0 first diter')
...
for i in range(3):
    n=i
    dss[i] = xr.open_dataset('../results.nc'%iters[i])
    N0=1e-3
    g=9.8
    alpha = 2e-4
    nz = dss[i].dims['Z']
    ...
    z=dss[i]['Z']
    if submean:
        v0[i] = (dss[i]['VVEL'].isel(Z=((z<-50.)&(z>-2000.))).mean()).values
        u0[i] = (dss[i]['UVEL'].isel(Z=((z<-50.)&(z>-2000.))).mean()).values-U0
    else:
        v0[i]=0.
        u0[i]=0.
    ...
    dss[n]['VVEL']=dss[n]['VVEL']-v0[i]
    dss[n]['UVEL']=dss[n]['UVEL']-u0[i]
...
for n in range(3):
    ...
    energy[n]['area']=dss[n]['rA'].sum().values
...
PL = xr.DataArray(0.5*(ds['PHIHYD'][:,1:,:,:].data+ds['PHIHYD'][:,:-1,:,:].data),dims=('time','Zl','YC','XC'),
                        coords={'Zl':ds['Zl'][1:].values,'YC':ds['YC'].values,'XC':ds['XC'].values })

print('Starting dWPz')
with ProgressBar():
    WP=((PL*ds['WVEL']*ds['rA']).sum(dim=('YC','XC'))).values
...
## sizes wrong here....
print(ds['V'])
ds['VC'] = xr.DataArray(0.5*(ds['VVEL'] + ds.VVEL.roll(YG=1)).data,
    dims=('time','Z','YC','XC'))

xx=(ds['VC']*ds['hFacC']*f0*U0*ds['rA']).sum(dim=('XC','YC'))
with ProgressBar():
...
if 1:
    for n in range(3):
        ...
        with ProgressBar():
            print(dss[n]['UVEL'])
            xx = (0.5*(dss[n]['UVEl']**2 * dss[n]['hFacW'].data) *
                    dss[n]['rA'].data
                 ).sum(dim=('YC','XG')).compute()
            xx += (0.5*(dss[n]['VVEL']**2 * dss[n]['hFacS'].data) *
                   dss[n]['rA'].data
                  ).sum(dim=('YG','XC')).compute()
        energy[n]['KE']=xx
        print(energy[n].keys())
        with ProgressBar():
            energy[n]['PE'] = (g*(4e-4*(dss[n]['THETA']-dss[n]['T0']))**2/2./N0**2*dss[n]['hFacC']*dss[n]['rA']).sum(dim=('XC','YC')).data.compute()
        print(energy[n].keys())
...
dEdt = energy[-1]['KE'][0] - energy[0]['KE'][0]
dEdt += (energy[-1]['PE'][0] - energy[0]['PE'][0])*9.8/4.0
dEdt = dEdt/3600.

resid = -dEdt+energy[1]['Bf'][0]-energy[1]['dWPdz'][0]

ennc = xr.Dataset(data_vars={'Bf': (['Z'], energy[1]['Bf'][0]),
                            'dWPdz': (['Z'], energy[1]['dWPdz'][0]),
                            'eps': (['Z'], energy[1]['eps'][0]),
                            'dEdt': (['Z'], dEdt),
                            'resid': (['Z'], resid),
                            },
                 coords={'Z': (energy[1]['Z'])})
name='EnergyDemean'+pre+'%010d.nc'%iters[1]
ennc.to_netcdf(name)
